...
SparseDenseSDMemory::SparseDenseSDMemory(id_t id, std::string name, Config stonne_cfg, Connection* write_connection, SST_STONNE::LSQueue* load_queue_, SST_STONNE::LSQueue* write_queue_, SimpleMem*  mem_interface_) : MemoryController(id, name) {
...
void SparseDenseSDMemory::cycle() {
    ...
    {    //Initialize these for the first time
     ...
     STONNE_Tile* tile1 = new STONNE_Tile(1, 1, T_K, T_N, 1, 1, 1, 1, false);
