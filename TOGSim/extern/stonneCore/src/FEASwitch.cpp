...
FEASwitch::FEASwitch(id_t id, std::string name, unsigned int level, unsigned int num_in_level, Config stonne_cfg) : Unit(id, name) {
    this->level = level;
    ...
    this->input_fw_below_nodes_fifo = new Fifo(buffers_capacity);
  //  std::cout << "Direccion de memoria antes fw fifo: " << this->input_fw_fifo << std::endl; 
    ...
    this->current_psum=0;
    this->forward_to_fold_node=false;
...
FEASwitch::FEASwitch(id_t id, std::string name, unsigned int level, unsigned int num_in_level, Config stonne_cfg, Connection* inputLeftConnection, 
Connection* inputRightConnection, Connection* forwardingConnection, Connection* outputConnection, Connection* memoryConnection) : FEASwitch(id, name, level, num_in_level, stonne_cfg) { //Constructor
...
FEASwitch::~FEASwitch() {
    ...
    delete this->input_fw_below_nodes_fifo;
...
void FEASwitch::setMemoryConnection(Connection* memoryConnection, unsigned int busID, unsigned int inputID) {
    this->busID = busID;
...
void FEASwitch::setUpNodeForwardingConnection(Connection* upNodeForwardingConnection) {         // Set the connection to the up node that manages the folding 
    this->upNodeForwardingConnection = upNodeForwardingConnection;
}
void FEASwitch::addInputConnectionToForward(Connection* inputConnectionToForward) {
    this->inputConnectionsToForward.push_back(inputConnectionToForward);
}
...
void FEASwitch::setForwardingToFoldNodeEnabled(bool forwarding_to_fold_node) {
    this->forward_to_fold_node = forwarding_to_fold_node;
}
...
void FEASwitch::send() {
    ...
    if(!output_fw_fifo->isEmpty()) {
        ...
        while(!output_fw_fifo->isEmpty()) { //TODO control bw 
            ...
            std::cout << "[ASWITCH_FUNC] Cycle " << local_cycle << ", FEASwitch " << this->level << ":" << this->num_in_level << " has sent a psum to the forwarding link" << std::endl;
#endif
   ...
   //     std::cout << "DEBUG GENERAL " << this->level << ":" << this->num_in_level << " at cycle " << this->local_cycle << std::endl;
        ...
        if(this->forward_to_memory) { //Optimization to send the psum to the memory once it is completed without traverse other adders con fw configuration
            assert(this->forward_to_fold_node==false);
#ifdef DEBUG_ASWITCH_FUNC
            std::cout << "[ASWITCH_FUNC] Cycle " << local_cycle << ", FEASwitch " << this->level << ":" << this->num_in_level << " has sent a psum to memory (FORWARDING DATA)" << std::endl;
#endif
            this->aswitchStats.n_memory_send++; //Track the information
            this->memoryConnection->send(vector_to_send_parent); 
        }

        else if(this->forward_to_fold_node) {
            this->upNodeForwardingConnection->send(vector_to_send_parent);
        }
        else { 
#ifdef DEBUG_ASWITCH_FUNC
            std::cout << "[ASWITCH_FUNC] Cycle " << local_cycle << ", FEASwitch " << this->level << ":" << this->num_in_level << " has sent a psum to the parent" << std::endl;
#endif
...
#ifdef DEBUG_ASWITCH_FUNC
            std::cout << "[ASWITCH_FUNC] Cycle " << local_cycle << ", FEASwitch " << this->level << ":" << this->num_in_level << " has received a psum from input port 0" << std::endl;
#endif
...
#ifdef DEBUG_ASWITCH_FUNC
            std::cout << "[ASWITCH_FUNC] Cycle " << local_cycle << ", FEASwitch " << this->level << ":" << this->num_in_level << " has received a psum from input port 1" << std::endl;
#endif
...
#ifdef DEBUG_ASWITCH_FUNC
                std::cout << "[ASWITCH_FUNC] Cycle " << local_cycle << ", FEASwitch " << this->level << ":" << this->num_in_level << " has received a psum from the forwarding link" << std::endl;
#endif
...
void FEASwitch::receive_below_nodes_fwlinks() {
    unsigned int n_connections_with_data=0;
    for(int i=0; i<inputConnectionsToForward.size(); i++) {
        if(this->inputConnectionsToForward[i]->existPendingData()) {
            n_connections_with_data++;
            std::vector<DataPackage*> data_received_fw = this->inputConnectionsToForward[i]->receive();
            assert(data_received_fw.size() == 1);
            for(int i=0; i<data_received_fw.size(); i++) {
#ifdef DEBUG_ASWITCH_FUNC
                std::cout << "[ASWITCH_FUNC] Cycle " << local_cycle << ", FEASwitch " << this->level << ":" << this->num_in_level << " has received a psum from the forwarding link of the nodes below" << std::endl;
#endif

                input_fw_below_nodes_fifo->push(data_received_fw[i]);
            }
        }
...
DataPackage* FEASwitch::perform_operation_2_operands(DataPackage* pck_left, DataPackage* pck_right) {
...
#ifdef DEBUG_ASWITCH_FUNC
            std::cout << "[ASWITCH_FUNC] Cycle " << local_cycle << ", FEASwitch " << this->level << ":" << this->num_in_level << " has performed a 2:1 sum" << std::endl;
#endif
    ...
    DataPackage* result_pck = new DataPackage (sizeof(data_t), result, PSUM, this->level, pck_left->get_vn(), this->operation_mode, pck_left->getRow(), pck_left->getRow());  //TODO the size of the package corresponds with the data size
...
DataPackage* FEASwitch::perform_operation_3_operands(DataPackage* pck_left, DataPackage* pck_right, DataPackage* pck_forward) {
    ...
    switch(this->operation_mode) {
        case ADDER: //SUM
#ifdef DEBUG_ASWITCH_FUNC
            std::cout << "[ASWITCH_FUNC] Cycle " << local_cycle << ", FEASwitch " << this->level << ":" << this->num_in_level << " has performed a 3:1 sum" << std::endl;
#endif
...
void FEASwitch::route_fold_config() {
    assert(this->forward_to_memory);
    //Receiving psum package from fw links 
    DataPackage* pck_received;
    if(!input_fw_below_nodes_fifo->isEmpty()) {
        pck_received = input_fw_below_nodes_fifo->pop();
        DataPackage* result;
        ...
        if(this->current_psum == (this->n_psums-1)) {
            this->output_psum_fifo->push(this->temporal_register);
            this->current_psum = 0;
...
void FEASwitch::printStats(std::ofstream& out, unsigned int indent) {
    out << ind(indent) << "{" << std::endl; //TODO put ID
...
void FEASwitch::printEnergy(std::ofstream& out, unsigned int indent){
    /* 
