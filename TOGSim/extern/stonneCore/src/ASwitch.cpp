...
/* This class represents the ASwitch of the MAERI architecture. Basically, the class contains to connections, which   */

ASwitch::ASwitch(id_t id, std::string name, unsigned int level, unsigned int num_in_level, Config stonne_cfg) : Unit(id, name) {
    this->level = level;
...
void ASwitch::setInputLeftConnection(Connection* inputLeftConnection) {
    this->inputLeftConnection = inputLeftConnection;
}

void ASwitch::setInputRightConnection(Connection* inputRightConnection) {
    this->inputRightConnection = inputRightConnection;
}

void ASwitch::setForwardingConnection(Connection* forwardingConnection) {
    this->forwardingConnection = forwardingConnection;
}

void ASwitch::setOutputConnection(Connection* outputConnection) {
    this->outputConnection = outputConnection;
}

void ASwitch::setMemoryConnection(Connection* memoryConnection,  unsigned int busID, unsigned int inputID) {
    this->busID = busID;
...
void ASwitch::setForwardingLinkDirection(fl_t fl_direction) {
...
void ASwitch::setConfigurationMode(adderconfig_t config_mode) {
...
void ASwitch::setOperationMode(adderoperation_t operation_mode) {
    this->operation_mode = operation_mode;
}

void ASwitch::setForwardingToMemoryEnabled(bool forwarding_to_memory) {
...
void ASwitch::send() {
    ...
    if(!output_fw_fifo->isEmpty()) {
        ...
        while(!output_fw_fifo->isEmpty()) { //TODO control bw
            ...
            std::cout << "[ASWITCH_FUNC] Cycle " << local_cycle << ", ASwitch " << this->level << ":" << this->num_in_level << " has sent a psum to the forwarding link" << std::endl;
#endif
...
#ifdef DEBUG_ASWITCH_FUNC
            std::cout << "[ASWITCH_FUNC] Cycle " << local_cycle << ", ASwitch " << this->level << ":" << this->num_in_level << " has received a psum from input port 1" << std::endl;
#endif
...
/*
    if((level == 8) && (num_in_level == 181)) {  // 5 y 22 es el 2:1 que va al vecino 21
        ...
        std::cout << "ASwitch type is: " << get_string_adder_configuration(config_mode) << std::endl;
    }
...
#ifdef DEBUG_ASWITCH_FUNC
                std::cout << "[ASWITCH_FUNC] Cycle " << local_cycle << ", ASwitch " << this->level << ":" << this->num_in_level << " has received a psum from the forwarding link" << std::endl;
#endif
...
DataPackage* ASwitch::perform_operation_2_operands(DataPackage* pck_left, DataPackage* pck_right) {
...
*/
DataPackage* ASwitch::perform_operation_3_operands(DataPackage* pck_left, DataPackage* pck_right, DataPackage* pck_forward) {
...
//Print the configuration of the ASwitch
void ASwitch::printConfiguration(std::ofstream& out, unsigned int indent) {
