...
SparseFlex_MSNetwork::SparseFlex_MSNetwork(id_t id, std::string name, Config stonne_cfg) : MultiplierNetwork(id, name) { 
...
void SparseFlex_MSNetwork::setMemoryConnections(std::vector<std::vector<Connection*>> memoryConnections) {
    ...
    unsigned int ms_per_line = this->ms_size / n_bus_lines;
    for(int i=0; i < this->ms_size; i++) {
...
void SparseFlex_MSNetwork::virtualNetworkConfig(std::map<unsigned int,unsigned int> vn_conf) {
...
void SparseFlex_MSNetwork::fwLinksConfig(std::map<unsigned int, bool> ms_fwsend_enabled, std::map<unsigned int, bool> ms_fwreceive_enabled) {
...
void SparseFlex_MSNetwork::forwardingPsumConfig(std::map<unsigned int, bool> forwarding_psum_enabled) {
...
void SparseFlex_MSNetwork::directForwardingPsumConfig(std::map<unsigned int, bool> direct_forwarding_psum_enabled) {
...
void SparseFlex_MSNetwork::nWindowsConfig(unsigned int n_windows) {
    for(int i=0; i<this->ms_size; i++) {
        SparseFlex_MSwitch* ms = this->mswitchtable[i];
...
void SparseFlex_MSNetwork::configureSparseSignals(std::vector<SparseVN> sparseVNs, DNNLayer* dnn_layer, unsigned int ms_size) {
    CompilerMSN* compiler_msn = new CompilerMSN();
...
void SparseFlex_MSNetwork::nFoldingConfig(std::map<unsigned int, unsigned int> n_folding_configuration) {
...
void SparseFlex_MSNetwork::printConfiguration(std::ofstream& out, unsigned int indent) {
...
void SparseFlex_MSNetwork::printStats(std::ofstream &out, unsigned int indent) {
