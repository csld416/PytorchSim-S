...
SparseFlex_MSwitch::SparseFlex_MSwitch(id_t id, std::string name, int num, Config stonne_cfg) : 
Unit(id, name){
    this->num = num;
...
SparseFlex_MSwitch::SparseFlex_MSwitch(id_t id, std::string name, int num, Config stonne_cfg, Connection* outputConnection, Connection* inputConnection) : SparseFlex_MSwitch(id, name, num, stonne_cfg) {
...
void SparseFlex_MSwitch::setOutputConnection(Connection* outputConnection) { //Set the left connection of the switch
    this->outputConnection = outputConnection;
}
void SparseFlex_MSwitch::setInputForwardingConnection(Connection* inputForwardingConnection) { //Set the right connection of the switch
    this->inputForwardingConnection = inputForwardingConnection;
}

void SparseFlex_MSwitch::setOutputForwardingConnection(Connection* outputForwardingConnection) {
    this->outputForwardingConnection = outputForwardingConnection;
}

void SparseFlex_MSwitch::setMemoryConnection(Connection* memoryConnection) {
    this->memoryConnection = memoryConnection;
}

void SparseFlex_MSwitch::setInputConnection(Connection* inputConnection) { //Set the input connection of the switch
    this->inputConnection = inputConnection;
}

void SparseFlex_MSwitch::setNWindows(unsigned int n_windows) {
    this->n_windows = n_windows;
#ifdef DEBUG_MSWITCH_CONFIG
    std::cout << "[MSWITCH_CONFIG] SparseFlex_MSwitch " << this->num << ". n_windows: " << this->n_windows << std::endl;
#endif
}

void SparseFlex_MSwitch::setNFolding(unsigned int n_folding) {
    this->n_folding = n_folding;
#ifdef DEBUG_MSWITCH_CONFIG
    std::cout << "[MSWITCH_CONFIG] SparseFlex_MSwitch " << this->num << ". n_folding: " << this->n_folding << std::endl;
#endif
}

void SparseFlex_MSwitch::setForwardPsum(bool forward_psum) {
    this->forward_psum = forward_psum;
#ifdef DEBUG_MSWITCH_CONFIG
    std::cout << "[MSWITCH_CONFIG] SparseFlex_MSwitch " << this->num << ". forward_psum: " << this->forward_psum << std::endl;
#endif

}

void SparseFlex_MSwitch::setDirectForwardPsum(bool direct_forward_psum) {
    this->direct_forward_psum = direct_forward_psum;
#ifdef DEBUG_MSWITCH_CONFIG
    std::cout << "[MSWITCH_CONFIG] SparseFlex_MSwitch " << this->num << ". direct_forward_psum: " << this->direct_forward_psum << std::endl;
#endif
...
void SparseFlex_MSwitch::forward(DataPackage* activation_to_forward) {
...
#ifdef DEBUG_MSWITCH_FUNC
            std::cout << "[MSWITCH_FUNC] Cycle " << this->local_cycle << ", SparseFlex_MSwitch " << this->num << " has sent an IACTIVATION to the forwarding link" << std::endl;
#endif
...
void SparseFlex_MSwitch::setVirtualNeuron(unsigned int VN) {
    this->VN = VN; 
    this->mswitchStats.n_configurations++;
#ifdef DEBUG_MSWITCH_CONFIG
    std::cout << "[MSWITCH_CONFIG] SparseFlex_MSwitch " << this->num << ". VirtualNeuron: " << this->VN << std::endl;
#endif
...
void SparseFlex_MSwitch::setInputForwardingEnabled(bool inputForwardingEnabled) {
    this->inputForwardingEnabled = inputForwardingEnabled;
#ifdef DEBUG_MSWITCH_CONFIG
    std::cout << "[MSWITCH_CONFIG] SparseFlex_MSwitch " << this->num << ". inputForwardingEnabled: " << this->inputForwardingEnabled << std::endl;
#endif

}

void SparseFlex_MSwitch::setOutputForwardingEnabled(bool outputForwardingEnabled) {
    this->outputForwardingEnabled = outputForwardingEnabled;
#ifdef DEBUG_MSWITCH_CONFIG
    std::cout << "[MSWITCH_CONFIG] SparseFlex_MSwitch " << this->num << ". outputForwardingEnabled: " << this->outputForwardingEnabled << std::endl;
#endif
...
#ifdef DEBUG_MSWITCH_FUNC
            std::cout << "[MSWITCH_FUNC] Cycle " << this->local_cycle << ", SparseFlex_MSwitch " << this->num << " has received an IACTIVATION from the distribution network" << std::endl;
#endif
      //  std::cout << "[MSWITCH_FUNC] Cycle " << this->local_cycle << ", SparseFlex_MSwitch " << this->num << " has received an IACTIVATION from the distribution network. Data: "<< pck->get_data() << std::endl;
...
#ifdef DEBUG_MSWITCH_FUNC
                std::cout << "[MSWITCH_FUNC] Cycle " << this->local_cycle << ", SparseFlex_MSwitch " << this->num << " has received a WEIGHT from the distribution network" << std::endl;
#endif
...
DataPackage* SparseFlex_MSwitch::perform_operation_2_operands(DataPackage* pck_left, DataPackage* pck_right) {
    //Extracting the values
    data_t result; // Result of the operation
    result = pck_left->get_data() *  pck_right->get_data(); 
#ifdef DEBUG_MSWITCH_FUNC
    std::cout << "[MSWITCH_FUNC] Cycle " << this->local_cycle << ", SparseFlex_MSwitch " << this->num << " has performed a multiplication" << std::endl;
#endif
   // std::cout << "MSwitch " << this->num << " is multiplying. Weight=" << pck_right->get_data() << "  Activation=" << pck_left->get_data() << std::endl;
...
void SparseFlex_MSwitch::printConfiguration(std::ofstream& out, unsigned int indent) {
...
void SparseFlex_MSwitch::printStats(std::ofstream& out, unsigned int indent) {
    out << ind(indent) << "{" << std::endl; //TODO put ID
...
void SparseFlex_MSwitch::printEnergy(std::ofstream& out, unsigned int indent) {
    /*
         This component prints:
             - SparseFlex_MSwitch multiplications
             - SparseFlex_MSwitch forwardings
