...
private:
    ...
    std::map<std::pair<int, int>, SparseFlex_ASwitch* > aswitchtable; //Map with the switches of the topology. The connection among them will be different depending on the topology used
    ...
    std::vector<SparseFlex_ASwitch*> single_switches; //List of switches that are single reduction switches  (see blueSpec implementation. i.e., the do not have forwarding connections)
    std::vector<SparseFlex_ASwitch*> double_switches; //List of double switches that are double reduction switches (i.e., in bluespec implementation these are sw that have fw links).
...
public:
    SparseFlex_ASNetwork(id_t id, std::string name, Config stonne_cfg, Connection* output_connection);
    ...
    void printConfiguration(std::ofstream& out, unsigned int indent);  //This function prints the configuration of the SparseFlex_ASNetwork (i.e., SparseFlex_ASwitches configuration such as ADD_2_1, ADD_3_1, etc)
