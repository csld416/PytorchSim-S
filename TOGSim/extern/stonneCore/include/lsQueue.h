...
{
public:
    LSEntry(const uint32_t reqId, DataPackage* data_package , uint32_t write) :
            req_id_(reqId), data_package_(data_package), write_(write), ready_(0) {}
    ~LSEntry() {}

    uint32_t getReqId() const { return req_id_; }
...
public:
    ...
    uint32_t getNextCompletedEntry() const { return completed_memory_queue_.front(); }
    ...
    DataPackage* getEntryPackage( uint32_t id)
    {
        auto entry = completed_.find( id );
        if( entry != completed_.end() )
        {
    ...
    void setEntryPackage( uint32_t id, DataPackage* data_package )
    {
    ...
    void setEntryData( uint32_t id, data_t data ) {
        auto entry = pending_.find(id);
        if(entry != pending_.end()) {
    ...
    void setEntryReady( uint32_t id, uint32_t ready )
    {
        auto entry = pending_.find( id );
        if( entry != pending_.end() )
        {
...
private:
    ...
    std::map< uint32_t, LSEntry* > pending_;
    std::map< uint32_t, LSEntry*> completed_;
};
